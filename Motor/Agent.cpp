...
void Agent::checkTilePosition(const vector<string>& levelData,
	vector<glm::vec2>& collideTilePosition, float x, float y)
{
	...
	if (cornerPos.x < 0 || cornerPos.x >= 100 
		|| cornerPos.y <0 || cornerPos.y >= 100)
	{
		return;
	}
	// Temporalmente es pos = vacio
	string pos = "";
	if (pos != ".")
	{
		collideTilePosition.push_back(cornerPos * tile_with 
			+ glm::vec2(tile_with / 2.0f));
	}
}
...
bool Agent::collideWithLevel(const vector<string>& levelData)
{
	...
	checkTilePosition(levelData, collideTilePosition,
		position.x + AGENT_RADIUS, position.y);
	checkTilePosition(levelData, collideTilePosition,
